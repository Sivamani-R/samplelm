...
export const TeamLeadApprovalsPage = () => {
  ...
  const loadApprovals = async () => {
    try {
      ...
      setError(err.message || 'Unable to fetch approval requests.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    },
    {
      ...
      cell: (row) => (
        <div style={{ fontSize: '12px' }}>
          ...
          <div style={{ fontSize: '11px', color: 'var(--text-tertiary)' }}>Applied {new Date(row.appliedDate).toLocaleDateString()}</div>
        </div>
      )
    },
    {
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Review, approve, reject with mandatory justification, or request document clarification"
        breadcrumbs={[
        ...
        <div style={{ width: '200px' }}>
          <Select
            ...
            options={[
              ...
              { value: 'OVERDUE', label: 'Overdue SLA Escalations' },
      ...
      {/* Table */}
      {isLoading ? (
        <LoadingSpinner text="Fetching pending approvals queue..." />
      ) : error ? (
        ...
        <Table
          ...
          emptyMessage="No pending leave applications require your approval at this time."
        />
      )}
